...
const modelType = 'stroke-risk';
const config = modelConfig[modelType];

// Path to stroke model and scaler
const modelPath = path.join(__dirname, '..', '..', '..', 'trained_models', 'stroke_nn.h5');
const scalerPath = path.join(__dirname, '..', '..', '..', 'trained_models', 'stroke_scaler.pkl');

// Load metadata
const metadataPath = path.join(__dirname, '..', '..', '..', 'trained_models', 'stroke_metadata.json');
let metadata = null;
...
try {
  ...
  console.error('❌ Error loading stroke metadata:', error.message);
}
 ...
 */
export async function predictStrokeRisk(features) {
  try {
    ...
    return {
      ...
      modelInfo: {
        status: 'error',
        message: 'Failed to make stroke prediction'
      }
    };
  }
}